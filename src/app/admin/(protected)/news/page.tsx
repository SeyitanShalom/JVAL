  ...
  return (
    <div className="grid gap-6">
      <AdminPageHeader
        ...
        action={
          <AddButton label="Post" title="Create News Post" description="Publish a new tournament news article.">
            <NewsForm
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Posts" value={data.posts.length} detail={data.source === "database" ? "Database" : "Sample preview"} />
        <MetricCard label="Competitions linked" value={new Set(data.posts.map((p) => p.competitionId)).size} detail="Scoped articles" />
        <MetricCard label="Latest" value={latestPost ? new Date(latestPost.publishDate).toLocaleDateString("en-GB") : "—"} detail={latestPost?.title ?? "None"} />
      ...
      <div className="grid gap-3">
              ...
              <div className="p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-blue-600">{post.competitionName}</p>
                    <h2 className="mt-1 text-base font-bold text-slate-950">{post.title}</h2>
                    <p className="mt-2 text-sm font-semibold leading-6 text-slate-600 line-clamp-2">{post.excerpt}</p>
                    <p className="mt-2 text-xs font-bold text-slate-400">{new Date(post.publishDate).toLocaleDateString("en-GB")}</p>
                  </div>
        ...
        {data.posts.length === 0 && (
          <div className="rounded-xl border border-dashed border-slate-200 p-10 text-center">
            <p className="text-sm font-bold text-slate-500">No posts yet. <span className="text-blue-600">Click "+ Post" to write the first article.</span></p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <form action={action} className="grid gap-4">
      <label className="grid gap-2 text-sm font-bold text-slate-700">
        Title
        <input name="title" defaultValue={post?.title} disabled={!canWrite} className="h-11 rounded-lg border border-slate-200 px-3 font-semibold outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100 disabled:bg-slate-100" placeholder="News title" />
      </label>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="grid gap-2 text-sm font-bold text-slate-700">
          Publish date
          <input type="date" name="publishDate" defaultValue={post?.publishDate.split("T")[0]} disabled={!canWrite} className="h-11 rounded-lg border border-slate-200 px-3 font-semibold outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100 disabled:bg-slate-100" />
        </label>
        <label className="grid gap-2 text-sm font-bold text-slate-700">
          Competition
          <select
      ...
      <label className="grid gap-2 text-sm font-bold text-slate-700">
        Content
        <textarea name="content" defaultValue={post?.content} disabled={!canWrite} className="min-h-32 rounded-lg border border-slate-200 px-3 py-3 font-semibold leading-6 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100 disabled:bg-slate-100" placeholder="Write match report or tournament update" />
      </label>
      <ImageUploadInput
        name="coverImageUrl"
...
function getPageMessage(
  ...
  if (query.created) return { tone: "success" as const, text: "Post published." };
  ...
  if (query.error === "missing") return { tone: "warning" as const, text: "Title, competition, and season are required." };
  if (query.error === "database") return { tone: "warning" as const, text: "Database not connected." };
  if (query.error === "save") return { tone: "warning" as const, text: "Could not save post." };
  if (query.error === "delete") return { tone: "warning" as const, text: "Could not delete post." };
