...
export default async function CompetitionDetailsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <section className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-8 sm:px-6">
      <div className="rounded-lg bg-blue-800 p-6 text-white md:p-8">
        <p className="text-xs font-black uppercase tracking-[0.2em] text-blue-100">
          {competition.type} | {competition.status}
        </p>
        <h1 className="mt-3 text-3xl font-black tracking-normal sm:text-5xl">{competition.name}</h1>
        <p className="mt-4 max-w-3xl text-sm font-semibold leading-7 text-white/85">{competition.description}</p>
            ...
            <div className="grid gap-2">
              {competitionTeams.map((team) => (
                <Link
                  ...
                  className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-3 text-sm font-black text-slate-950"
                >
                  <span>{team.name}</span>
      ...
      <Link href="/competitions" className="inline-flex items-center gap-2 text-sm font-black text-blue-700">
