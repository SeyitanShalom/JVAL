...
import { formatDate, getCompetitionById, type NewsPost } from "@/lib/league-data";
...
export default function NewsCard({ post, large = false }: NewsCardProps) {
  ...
  return (
    <Link
      href={`/news/${post.slug}`}
      className={`grid overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition hover:border-blue-500 hover:shadow-md ${
    ...
    >
      ...
      <div className="p-4">
        <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] font-bold uppercase text-blue-600">
