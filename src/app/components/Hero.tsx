...
export default function Hero({
  ...
  return (
    <section className="motion-hero animated-gradient-hero -mx-4 border-y border-red-100 bg-white px-4 py-5 sm:-mx-6 sm:px-6 lg:py-7">
      <HeroAutoScroll />
      <div className="mx-auto grid max-w-6xl items-center gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:gap-10">
        <div className="motion-hero-copy min-w-0 text-center lg:text-left">
          ...
          <p className="mx-auto mt-3 max-w-xl text-sm font-semibold leading-6 text-slate-600 lg:mx-0">
            ...
            stats, and league tables all in one place. Football lives here.
          </p>

          <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center lg:justify-start">
            <Link
              href="/fixtures"
              className="motion-button gradient-cta inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-red-500 px-4 text-xs font-bold text-white shadow-sm transition hover:bg-red-600"
            >
              View fixtures
          ...
          <div className="motion-stagger mt-5 grid grid-cols-3 gap-2 text-left">
            {stats.map((item) => (
              <div
                key={item.label}
                className="motion-card hero-stat rounded-lg bg-red-50 px-3 py-2"
              >
