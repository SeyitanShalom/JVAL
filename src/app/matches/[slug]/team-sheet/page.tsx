      ...
      >
        ...
        <div className="space-y-1 border-b-2 border-slate-950 pb-4 text-center print:border-black">
          ...
          <p className="text-sm font-semibold text-slate-600">
            {formatDate(match.date)} &middot; {formatMatchTime(match.date)}{" "}
            &middot; {venue.name}, {venue.location}
          </p>
          {match.referee && (
              ...
              {players.length > 0 ? (
                <div className="space-y-3">
                  {POSITION_ORDER.map((group) => {
                    const grouped = players.filter(
                      (p) => p.positionGroup === group,
                    );
                    if (!grouped.length) return null;
                    return (
                      <div key={group}>
                        <p className="mb-1 text-[9px] font-bold uppercase tracking-[0.08em] text-slate-400">
                          {group}s
                        </p>
                        <table className="w-full border-collapse text-xs">
                          <tbody>
                            {grouped.map((player) => (
                              ...
                              >
                                <td className="w-8 py-1.5 font-bold text-slate-950">
                                  {player.number}
                                </td>
                                <td className="py-1.5 font-semibold text-slate-800">
                                  {player.name}
                                </td>
                                <td className="py-1.5 text-right text-[10px] font-bold text-slate-400">
        ...
        <div className="rounded-xl border border-slate-200 p-4 text-xs font-semibold text-slate-600">
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            <p>
              <span className="font-bold text-slate-950">Kickoff: </span>
